...
class Effect15(BaseEffect):
            ...
            points2 = np.array(
                [
                    [
                        ...
                        (
                            int(landmark.y * height) + self.i
                            if i in self.e_indices
                            else int(landmark.y * height) - self.i
                        ),
                    ]
                    for i, landmark in enumerate(results.multi_face_landmarks[0].landmark)
                    if i in self.indices
    ...
    def settings(self, settings_dict: dict):
        ...
        self.indices = [71,68,104,69,108,337,299,333,298,301]
        for m in meshes:
            for i, ii in m:
                self.indices.append(i)
                self.indices.append(ii)

        self.e_indices = [71,68,104,69,108,337,299,333,298,301]
        for m in meshes[1:]:
            for i, ii in m:
                self.e_indices.append(i)
                self.e_indices.append(ii)
    ...
    def set_prikol_on_img(self, img: np.ndarray) -> np.ndarray:
        ...
        new_face = None
        try:
            res, res2 = self.detection(img)

            my_face, my_face_pts, y_min, y_max, x_min, x_max = self.crop_face(img, res)
            _, t_pts, _, _, _, _ = self.crop_face(img, res2)
            new_face = (
                trans(
                    t_pts,
                    my_face_pts,
