...
class Effect1(BaseEffect):
    ...
    def settings(self, settings_dict: dict[str, str]):
        ...
        self.y = 0
        self.ch = 0
        self._filter = np.zeros(shape=(self.square_size, self.square_size, 3), dtype=np.uint8)
        self.face_img = np.zeros((self.square_size, self.square_size, 3), dtype=np.uint8)
    ...
    def set_prikol_on_img(self, img: np.ndarray) -> np.ndarray:
        ...
        if face_img is None or not all(face_img.shape):
        ...
        self.screen[self.y : self.y + self.square_size, self.x : self.x + self.square_size] = face_img
