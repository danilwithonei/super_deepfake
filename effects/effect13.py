...
# copied from https://github.com/torrober/vhs.py/blob/main/vhs.py
class Effect13(BaseEffect):
    def __init__(self) -> None:
        super().__init__()
        self._settings_dict = {}
        self.is_ready = False
        self.lumaCompressionRate = 2
        self.lumaNoiseSigma = 13
        self.lumaNoiseMean = 0
        self.chromaCompressionRate = 15
        self.chromaNoiseIntensity = 20
        self.verticalBlur = 1
        self.horizontalBlur = 2
        self.borderSize = 1.7 / 100
        self.generation = 3
        self.time_start = time.time()
        font_path = "fonts/VCR_OSD_MONO.ttf" 
        font_size = 36
    ...
    def addChromaNoise(self, image, intensity=10):
        ...
        noise_red = np.random.randint(
            -intensity, intensity, (height, width), dtype=np.int16
        )
        noise_green = np.random.randint(
            -intensity, intensity, (height, width), dtype=np.int16
        )
        noise_blue = np.random.randint(
            -intensity, intensity, (height, width), dtype=np.int16
        )
    ...
    def compressLuma(self, image):
        ...
        step1 = cv2.resize(
            image,
            (int(width / self.lumaCompressionRate), int(height)),
    ...
    def compressChroma(self, image):
        ...
        step1 = cv2.resize(
            image,
            (int(width / self.chromaCompressionRate), int(height)),
            interpolation=cv2.INTER_LANCZOS4,
        )
    ...
    def waves2(self, img):
        ...
        offset_x = (
            (
                waves
                * np.sin(np.cos(random.randint(200, 250)) * 2 * np.pi * i / (2 * cols))
            )
        ).astype(int)
        offset_j = j + offset_x
    ...
    def switchNoise(self, img):
        ...
        # Calculate the offset for each pixel
        offset_x = (waves * np.sin(np.cos(250) * 2 * np.pi * i / (2 * cols))).astype(
            int
        )
    ...
    def processFrame(self, image):
        ...
        chrominance_bgr[:, -stripe_width:, 1] = (
            0  # Set the green channel to 0 in the specified region
        )
    ...
    def yiq2bgr(
        self, yiq: np.ndarray, dst_bgr: np.ndarray = None, field: int = 0
    ) -> np.ndarray:
        c, h, w = yiq.shape
    ...
    def add_datetime(self, image):
        elapsed_time = time.time() - self.time_start
        hours = int(elapsed_time // 3600)
        minutes = int((elapsed_time % 3600) // 60)
        seconds = int(elapsed_time % 60)
        image_height, image_width, _ = image.shape
        x = image_width - image_width // 3
        y = image_height - image_height // 10

        image = Image.fromarray(image)
        draw = ImageDraw.Draw(image)
        draw.text((x, y), f"{hours:02}:{minutes:02}:{seconds:02}", fill=(255, 255, 255), font=self.font)


        now = datetime.now()
        td = now.strftime("%I:%M AM_.%d.%Y")
        t, d = td.split("_")
        draw.text((10, image_height - 80), t, fill=(255, 255, 255), font=self.font)
        d = months[now.month - 1][:3] + d
        draw.text((10, y), d, fill=(255, 255, 255), font=self.font)

        image = np.asarray(image)
        return image

    def processAll(self, image):
        image = self.add_datetime(image)
